...
public static class HelperMethods
{
    ...
    //gets components of type t at box with center point, size and angle. it will return true if at least one found and the found components are returned in the list
    public static bool GetComponentsAtBoxLocation<T>(out List<T> listComponentsAtBoxPosition, Vector2 point, Vector2 size, float angle)
    {
        ...
        Collider2D[] collider2DArray = Physics2D.OverlapBoxAll(point, size, angle);//world position = point, size = cursor size, angle = bounds of the box
