...
public class RoomTemplateSO : ScriptableObject
{
    ...
    //will prob be taken out as music should switch based off dungeon level
    #region Tooltip
    ...
    #region Tooltip
    [Tooltip("If you can imagine a rectangle around the room tilemap that just completely encloses it, the room lower bounds represent the bottom left corner of that rectangle. This shoulc be determined from the tilemap for the room (using the coordinate brush pointer to get the tilemap grid position for that bottom left corner (Note: this is the local tilemap position and NOT the world position")]
    #endregion Tooltip
    ...
    #region Tooltip 
    [Tooltip("Populate the list with all the enemies that can be spawned in this room by dungeon level, including the ratio (randomness) of this enemy type will be spawned in")]
    #endregion Tooltip 
    ...
    //I will be deleting my other spawner script eventually as this one is far more reliable + professional
    ...
    private void OnValidate() 
    {
        ...
        HelperUtilities.ValidateCheckNullValue(this, nameof(battleMusic), battleMusic);//prob remove
        HelperUtilities.ValidateCheckNullValue(this, nameof(ambientMusic), ambientMusic);//prob remove
        ...
        if(enemiesByLevelList.Count > 0 || roomEnemySpawnParametersList.Count > 0)
        {
            ...
            foreach(RoomEnemySpawnParameters roomEnemySpawnParameters in roomEnemySpawnParametersList)
            {
                ...
                HelperUtilities.ValidateCheckPositiveRange(this, nameof(roomEnemySpawnParameters.minTotalEnemiesToSpawn), roomEnemySpawnParameters.minTotalEnemiesToSpawn, nameof(roomEnemySpawnParameters.maxTotalEnemiesToSpawn),
                roomEnemySpawnParameters.maxTotalEnemiesToSpawn, true);
                HelperUtilities.ValidateCheckPositiveRange(this, nameof(roomEnemySpawnParameters.minSpawnInterval), roomEnemySpawnParameters.minSpawnInterval, nameof(roomEnemySpawnParameters.maxSpawnInterval),
                roomEnemySpawnParameters.maxSpawnInterval, true);                          
                HelperUtilities.ValidateCheckPositiveRange(this, nameof(roomEnemySpawnParameters.minConcurrentEnemies), roomEnemySpawnParameters.minConcurrentEnemies, nameof(roomEnemySpawnParameters.maxConcurrentEnemies),
                ...
                {
                    Debug.Log("No enemy types have been specified in for the dungeon level |" + roomEnemySpawnParameters.dungeonLevel.levelName + "| in gameobject " + this.name.ToString());
                }
