...
public class DungeonBuilder : SingletonMonobehaviour<DungeonBuilder>
{
  ...
  private void OnDisable()
  {
    ...
    //set dimmed material to be fully visible
  ...
  protected override void Awake() 
  {
    ...
    //generate random dungeon, returns true if dungeon built, but false if failed
    ...
    //attempt to randomly build the dungeon for the specified room node graph, returns true if a successful random layout
    //was generated, but returns false if a problem was encountered and another attempt is required.
    ...
    private bool CanPlaceRoomWithNoOverlaps(RoomNodeSO roomNode, Room parentRoom)
    {
      ...
      //do while room overlaps - try to place against all available doorways of the parent until the room is
      //successfully placed without overlap
      while(roomOverlaps)
      {
        ...
        //get a random room template for a room node that is consistent with the parent door orientation
    ...
    private RoomTemplateSO GetRandomTemplateForRoomConsistentWithParent(RoomNodeSO roomNode, Doorway doorwayParent)
    {
      ...
      //if room node is a corridor then select random correct corridor room template based on the parent doorway orientation
    ...
    //place the room - return true if the room does not overlap, false otherwise
    private bool PlaceTheRoom(Room parentRoom, Doorway doorwayParent, Room room)
    {
      ...
      //calculate adjustment position offset based on room doorway position that we are trying to connect(e.g. if this doorway is west then I need
    ...
    //check for rooms that overlap the upper and lower parameters, and if there are overlapping rooms then return room else return null
    ...
    //check if 2 rooms overlap each other - return true if they overlap or false if they do not
    ...
    //get a random room template from the roomtemplatelist that matches the roomType and returns it
    //(return null if no matching room templates are found).
    ...
    private Room CreateRoomFromRoomTemplate(RoomTemplateSO roomTemplate, RoomNodeSO roomNode)
    {
      ...
      room.ambientMusic = roomTemplate.ambientMusic; //prob remove
      room.roomNodeType = roomTemplate.roomNodeType; //prob remove
      ...
      if(roomNode.parentRoomNodeIDList.Count == 0) //Entrance
      {
        ...
        OldGameManager.Instance.SetCurrentRoom(room);
        
      }
      else 
      {
        room.parentRoomID = roomNode.parentRoomNodeIDList[0];//this is making only 1 number in the INDEX (starts at 0)
      }

      //if there are no enemies to spawn then default the room to be clear of enemies
      if(room.GetNumberOfEnemiesToSpawn(OldGameManager.Instance.GetCurrentDungeonLevel()) == 0)
      {
    ...
    //select a random node graph from the list of room node graphs
    ...
    //get room by roomID, if no room exists that ID return null
