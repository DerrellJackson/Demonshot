...
public class GridPropertiesManager : SingletonMonobehaviour<GridPropertiesManager>, ISaveable
{
    ...
    [SerializeField] private CropDetailsListSO cropDetailsListSO = null;
    [SerializeField] private GridPropertiesSO[] gridPropertiesSOArray = null;
    ...
    private void OnEnable() 
    {
        ...
        StaticEventHandler.AfterSceneLoadEvent += AfterSceneLoaded;

        StaticEventHandler.AdvanceGameDayEvent += AdvanceDay;
   ...
   public void DisplayWateredGround(GridPropertyDetails gridPropertyDetails)
    ...
    private void ConnectDugGround(GridPropertyDetails gridPropertyDetails)
    {
        ...
        //set 4 tiles if dug surrounding current tile, up, down, left, right since the central tile has been dug
    ...
    private void ConnectWateredGround(GridPropertyDetails gridPropertyDetails)
    {
        ...
        //select tile based on surrounding watered tile
        ...
        //set 4 tiles if water surrounding current tile, up, down, left, right since the central tile has been watered
        GridPropertyDetails adjacentGridPropertyDetails;
    ...
    private Tile SetDugTile(int xGrid, int yGrid)
    {
        ...
        //get surrounding tiles and if they are dug or not
        bool upDug = IsGridSquareDug(xGrid, yGrid + 1);
        bool downDug = IsGridSquareDug(xGrid, yGrid - 1);
        bool leftDug = IsGridSquareDug(xGrid - 1, yGrid);
        bool rightDug = IsGridSquareDug(xGrid + 1, yGrid);

        #region Set appropriate tile based on what surrounding tiles are dug or not
        ...
        #endregion Set appropriate tile based on what surrounding tiles are dug or not
    ...
    private Tile SetWateredTile(int xGrid, int yGrid)
    {
        ...
        //get surrounding tiles and if they are watered or not
        ...
        #region Set appropriate tile based on what surrounding tiles are watered or not
        ...
        #endregion Set appropriate tile based on what surrounding tiles are watered or not
    }
    ...
    public void DisplayPlantedCrop(GridPropertyDetails gridPropertyDetails)
    {
        ...
        {
            //get crop details
            CropDetails cropDetails = cropDetailsListSO.GetCropDetails(gridPropertyDetails.seedItemCode);
            ...
            worldPosition = new Vector3(worldPosition.x + Settings.gridCellSize / 2, worldPosition.y + 0.5f, worldPosition.z);
    ...
    //this initialises the grid property dictionary with the values from the gridPropertiesSO and stores the values for each scene in the GameObjectSave sceneData
    private void InititialiseGridProperties()
    {

        //loop through all gridproperties in the array 
        foreach(GridPropertiesSO gridPropertiesSO in gridPropertiesSOArray)
        {
            //create dictionary at grid property details 
            Dictionary<string, GridPropertyDetails> gridPropertyDictionary = new Dictionary<string, GridPropertyDetails>();
            
            //populate grid property dictionary, it will iterate through all the grid properties in the gridpropertiesSo list
            foreach(GridProperty gridProperty in gridPropertiesSO.gridPropertyList)
            {
            ...
            //create scene save for this object
            SceneSave sceneSave = new SceneSave();

            //add grid property dictionary to scene save data 
            sceneSave.gridPropertyDetailsDictionary = gridPropertyDictionary;

            //if starting scene set the grid property dictionary member variable to the current iteration 
            if(gridPropertiesSO.sceneName.ToString() == SceneControllerManager.Instance.startingSceneName.ToString())
            {
                this.gridPropertyDictionary = gridPropertyDictionary;
            }

            //add scene save to game object scene data 
            GameObjectSave.sceneData.Add(gridPropertiesSO.sceneName.ToString(), sceneSave);
        }
    ...
    //returns the grid property details at the grid location for the supplied dictionary, or null if no properties exist at that location
    ...
    public CropDetails GetCropDetails(int seedItemCode)
    {
        ...
        return cropDetailsListSO.GetCropDetails(seedItemCode);

    }


    //get the grid property details for the tile at gridX and gridY, if no property details exist then null is returned and can assume that all detail values are null or false
    ...
    public void ISaveableRestoreScene(string sceneName)
    {
        ...
        //get scene save for the scene, it exists bc it was created in initialise
        if(GameObjectSave.sceneData.TryGetValue(sceneName, out SceneSave sceneSave))
        {
            //get grid property details disctionary, it exists bc it was created in initialise
    ...
    //set the grid property details for the tile at gridX and gridY for the current scene 
    ...
    //set grid property details for the tile at gridX and gridY for the grid property dictionary
    ...
    private void AdvanceDay(int gameYear, Season gameSeason, int gameDay, string gameDayOfWeek)//, int gameHour, int gameMinute, int gameSecond)
    {
        ...
        foreach(GridPropertiesSO gridPropertiesSO in gridPropertiesSOArray)
        {
            ...
            if(GameObjectSave.sceneData.TryGetValue(gridPropertiesSO.sceneName.ToString(), out SceneSave sceneSave))
            {
                if(sceneSave.gridPropertyDetailsDictionary != null)
                {
                    for(int i = sceneSave.gridPropertyDetailsDictionary.Count - 1; i >= 0; i--)
                    {
                        ...
                        //ElementAt is an extension method that lets u get an element at a specific spot in the dictionary

                        GridPropertyDetails gridPropertyDetails = item.Value;

                        #region Update all grid property values to reflect the advance in the day
                        ...
                        //if ground is watered then it is not
                        if(gridPropertyDetails.daysSinceWatered > -1)
                        {
                        ...
                        #endregion Update all grid property values to reflect the advance in the day
                    }
                }
            }
        }

        //display grid property details to reflect changed value 
        DisplayGridPropertyDetails();
